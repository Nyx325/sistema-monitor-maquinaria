...
import { NewFuelUsedLast24 } from "../../model/entities/NewFuelUsedLast24.js";
import { ApiSnapshot, Equipement, FuelUsedLast24 } from "@prisma/client";
import { IFuelRepository } from "../../model/repository/use_cases/IFuelRepository.js";
import { PrismaFuelUsedLast24Repository } from "../../model/repository/infraestructure/PrismaFuelUsedLast24Repository.js";
...
export class FuelUsedLast24Controller extends Controller {
  private readonly repo: IFuelRepository = new PrismaFuelUsedLast24Repository();
  ...
  protected async performAdd(r: Request): Promise<void> {
    ...
    const fuel: Partial<NewFuelUsedLast24> = {
      fuel_consumed:
    ...
    const keys = Object.keys(fuel) as Array<keyof NewFuelUsedLast24>;
    ...
    await this.repo.add(fuel as NewFuelUsedLast24);
  }
  ...
  protected async performUpdate(r: Request): Promise<void> {
    ...
    const fuel: Partial<FuelUsedLast24> = {
      fuel_units: fuel_units !== undefined ? String(fuel_units) : undefined,
    ...
    const keys = Object.keys(fuel) as Array<keyof FuelUsedLast24>;
    for (const key of keys) {
      if (key === "date_time") continue;
    ...
    await this.repo.update(fuel as FuelUsedLast24);
  }
  ...
  protected async performGetBy(r: Request): Promise<unknown> {
    ...
    const criterio: Partial<FuelUsedLast24> = {
      active: active !== "false",
      fuel_consumed:
        fuel_consumed !== undefined ? Number(fuel_consumed) : undefined,
      fuel_units: fuel_units !== undefined ? String(fuel_units) : undefined,
    ...
    const keys = Object.keys(criterio) as Array<keyof Partial<FuelUsedLast24>>;
